...
How to connect this frontend to the real Social Scales SMMA backend, and what the
backend has to provide.

The short version: **implement the endpoints listed below, set two environment
variables, restart.** No React component changes.
...
SocialScalesAdapter                        src/lib/social-scales/adapter.ts
      ...
      └── HttpSocialScalesAdapter           http-adapter.ts   ← the file you edit
```
...
`resolveDataMode()` in `src/lib/social-scales/index.ts` reads these. `mock` is the
default when the variable is absent, so nothing breaks if it is unset.
...
## 5. Merging into the main SMMA repository

The frontend was built to be lifted, not rewritten.
...
**Option B — fold it into the backend's Next.js app** (recommended if the backend is
already Next.js with Prisma and server actions):

1. Copy `src/components`, `src/features`, `src/lib/social-scales`, `src/lib/display.ts`,
   `src/lib/utils.ts`, `src/lib/use-persistent-object.ts` and the route folders under
   `src/app/(app)/` into the backend app.
2. Copy the design tokens block from `src/app/globals.css`.
3. Write a **third adapter** — `PrismaSocialScalesAdapter` — implementing
   `SocialScalesAdapter` by calling the backend's existing services and repositories
   directly. No HTTP hop.
4. Register it in `getAdapter()` alongside the other two.
5. Delete `src/mocks/` and `mock-adapter.ts` once the real adapter is complete.

Point 3 is the whole payoff: the UI already speaks a vocabulary the backend can satisfy,
so integration is one file rather than a rebuild.
...
| Gap | Where it lands |
| --- | --- |
| Authentication and sessions | Host app; the topbar account menu is a placeholder and says so |
| Real OAuth for social platforms | Backend; `/integrations` renders whatever status the adapter reports |
| Generation-job polling / streaming | Studio — see section 4 |
| Media upload and asset storage | Backend; the Studio upload tile is disabled and labelled *backend only* |
