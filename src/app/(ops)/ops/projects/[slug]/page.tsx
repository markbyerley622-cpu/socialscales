...
export async function generateMetadata(
  props: PageProps<"/ops/projects/[slug]">,
): Promise<Metadata> {
...
export default async function ProjectPage(props: PageProps<"/ops/projects/[slug]">) {
  const { slug } = await props.params;
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <Link href="/ops/projects" className={buttonClass("ghost", "md")}>
              <ArrowLeft />
              All projects
            </Link>
            <Link
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
          <Card>
            <CardHeader
              ...
              action={
                <Link
                  href={`/ops/analytics?project=${project.slug}`}
        ...
        <div className="grid gap-4 xl:grid-cols-2">
            ...
            <ul className="divide-y divide-hairline">
              {recentPosts.map((post) => (
                <li key={post.id} className="flex flex-wrap items-center gap-2 px-4 py-2.5">
                  ...
                  <Link
                    href={`/ops/content/${post.asset.id}`}
          ...
          <Card>
            <CardHeader
              ...
              action={
                <Link
                  href={`/ops/activity?project=${project.slug}`}
                  className={buttonClass("ghost", "sm")}
                >
                  Full log
                  <ArrowUpRight />
